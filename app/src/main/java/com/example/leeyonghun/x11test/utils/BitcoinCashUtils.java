...
public class BitcoinCashUtils implements UtilsImpl{
        private  final String TAG = "BitcoinCashUtils";
        public  Context params = new Context(AppApplication.isMainnet ? MainNetParams.get() : TestNet3Params.get());
        private static BitcoinCashUtils instance = new BitcoinCashUtils();



        private BitcoinCashUtils(){}

        public static BitcoinCashUtils getInstance()
        {
            return instance;
        }
    ...
    public String createTransaction(String toAddress , String amount, ArrayList<UTXO> utxos, Wallet wallet){
        ...
        for(UTXO utxo : utxos)
        {
            ...
            tx.addSignedInput(outPoint,utxo.getValue(),utxo.getScript(), key, Transaction.SigHash.ALL, true,true);
            break;
        }
